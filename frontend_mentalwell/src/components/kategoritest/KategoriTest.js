...
const KategoriTest = () => {
  ...
  const deleteKategoriTest = async (id) => {
    try {
      ...
      if (confirmed) {
        await axios.get(`http://localhost:8080/api/kategori_test/delete/${id}`);
        getKategoriTests();
      }
    } catch (error) {
                ...
                <tbody>
                  {kategoriTests.map((kategoriTest, index) => (
                    <tr key={kategoriTest.id_test}>
                      ...
                      <td>
                        <Link
                          to={`/kategoritest/edit/${kategoriTest.id_test}`}
                        ...
                        <Button
                          variant="danger"
                          size="sm"
                          onClick={() => deleteKategoriTest(kategoriTest.id_test)}
                        >
