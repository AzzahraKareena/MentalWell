...
const EditKategoriTest = () => {
  ...
  const getKategoriTest = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/kategori_test/${id}`);
      const { nama_test, deskripsi_test, sub_test } = response.data;
      setNamaTest(nama_test);
      setDeskripsiTest(deskripsi_test);
      setSubTest(sub_test);
    } catch (error) {
  ...
  const updateKategoriTest = async (e) => {
    e.preventDefault();
    try {
      ...
      await axios.post(`http://localhost:8080/api/kategori_test/${id}`,formData ,{
        headers: {
      ...
      console.error("Error updating kategori test:", error);
    }
  };
    ...
    <AdminLayout>
      <div>
        <Breadcrumb className="mt-3">
          ...
          <Breadcrumb.Item active>Edit Kategori Test</Breadcrumb.Item>
        </Breadcrumb>
          ...
          <Card.Body>
            <div className="d-flex justify-content-between align-items-center mb-5">
              <h5 className="font-weight-bold" style={{ fontSize: "20px" }}>
                Form Edit Kategori Test
              </h5>
            </div>
            <Form onSubmit={updateKategoriTest}>
              <Row className="mb-3">
              ...
              <Row className="mb-3">
                <Col md="2" className="d-flex justify-content-end">
                  <Form.Label>Deskripsi Test</Form.Label>
                </Col>
                <Col md="8">
                  <Form.Control
                    as="textarea"
...
export default EditKategoriTest;
